...
class FileProcessor:
    ...
    def process_file(self, filepath: str) -> str:
        ...
        try:
            ...
            if cached_content:
                print(f"🚀 File Processor: Using cached content for "
    ...
    def _create_image_analysis_content(self, filepath: str, 
...
"""
    ...
    def _process_image(self, filepath: str) -> Dict[str, Any]:
        ...
        try:
            with Image.open(filepath) as img:
                ...
                return {
                    'type': 'image',
                    'text': (f'Image file: {img.format} format, '
        ...
        try:
            with open(filepath, 'rb') as file:
                ...
                for page_num, page in enumerate(pdf_reader.pages):
                    text_content.append(
                        f"Page {page_num + 1}: {page.extract_text()}"
                    )
                
                metadata = {
                    'num_pages': len(pdf_reader.pages),
                    'title': (pdf_reader.metadata.get('/Title', 'Unknown') 
                            if pdf_reader.metadata else 'Unknown'),
        ...
        try:
            ...
            try:
                with zipfile.ZipFile(filepath, 'r') as zip_file:
                    diagram_files = [f for f in zip_file.namelist() 
                                   if f.endswith('.xml')]
    ...
    def _process_vsdx(self, filepath: str) -> Dict[str, Any]:
        ...
        try:
            with zipfile.ZipFile(filepath, 'r') as zip_file:
                ...
                content_files = [f for f in zip_file.namelist() 
                               if 'content' in f.lower()]
                ...
                for content_file in content_files:
                    try:
                        ...
                        logger.error(
                            f"Unexpected error reading {content_file}: {e}"
                        )
                        continue
    ...
    def _extract_xml_text(self, element) -> str:
        ...
        if element.tag:
            tag_name = (element.tag.split('}')[-1] 
                       if '}' in element.tag else element.tag)
    ...
    def _extract_drawio_content(self, root) -> str:
        ...
        for cell in root.iter():
            if 'mxCell' in cell.tag:
                ...
                if style and ('azure' in style.lower() or 
                             'aws' in style.lower()):
    ...
    def _extract_svg_text(self, element) -> str:
        ...
        for text_elem in element.iter():
            tag_name = (text_elem.tag.split('}')[-1] 
                       if '}' in text_elem.tag else text_elem.tag)
            
            if tag_name in ['text', 'tspan', 'title', 'desc']:
                if text_elem.text and text_elem.text.strip():
                    text_parts.append(
                        f"[{tag_name}] {text_elem.text.strip()}"
                    )
